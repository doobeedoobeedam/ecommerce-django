...
class GirlListView(generic.ListView):
    template_name = 'girl_produk.html'
    queryset = ProdukItem.objects.filter(kategori='S')  # Filter hanya produk dengan kategori 'girl'
    paginate_by = 8

class BoyListView(generic.ListView):
    template_name = 'boy_produk.html'
    queryset = ProdukItem.objects.filter(kategori='OW')  # Filter hanya produk dengan kategori 'boy'
    paginate_by = 8

class NewListView(generic.ListView):
    template_name = 'new_produk.html'
    queryset = ProdukItem.objects.filter(label='NEW')  # Filter hanya produk dengan label 'NEW'
    paginate_by = 4

class BestListView(generic.ListView):
    template_name = 'best_produk.html'
    queryset = ProdukItem.objects.filter(label='BEST')  # Filter hanya produk dengan label 'best'
    paginate_by = 4
